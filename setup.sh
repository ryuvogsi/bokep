...
url_izin="https://raw.githubusercontent.com/bokep/izin/main/ip"
...
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "\033[5;36m│\e[0;35m             Agung Tunneling           \e[5;36m│\033[0m"
    echo -e " \033[5;36m───────────────────────────────────────\033[0m"
    echo -e ""
    echo -e " \033[0;32m You have installed the script\033[0m"
    echo -e ""
    echo -e "\033[5;36m ───────────────────────────────────────\033[0m "
    echo -e "\033[5;36m| \e[34mWhatsapp : +6287792681887            \e[5;36m | "
    echo -e "\033[5;36m| \e[34mTelegram : @AgungStores               \e[5;36m| "
    echo -e "\033[5;36m ───────────────────────────────────────\033[0m "
    echo -e ""
    exit 0
  fi
...
function pointing() (
    ...
    DOMAIN=ryuvogsi.shop
    generate_random_subdomain
    dns=${sub}.${DOMAIN}
    CF_KEY=b182d86b628441fbaf1672c5a49299ec9cf18
    CF_ID=bismillahbismillah2636@gmail.com
    set -euo pipefail
    echo ""
...
function pasang_domain() {
    #Membuat directory yang di perlukan
    mkdir -p /etc/xray
...
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
* * * * * root truncate -s 0 /var/log/syslog \
...
wget https://raw.githubusercontent.com/ryuvogsi/bokep/main/menu.zip
    wget -q -O /usr/bin/enc "https://cloud.agung-store.my.id/enc/enc" ; chmod 100 /usr/bin/enc
    7z e -paskyagung4sNvPBjIfZOsuSUfA2QbY3k2yDUUYF x menu.zip
    unzip menu.zip
    chmod 100 menu/*
...
insnoobz() {
wget https://cloud.agung-store.my.id/noobzvpns.zip
unzip noobzvpns.zip
cd noobzvpns
bash install.sh
cd
rm -rf noobzvpns.zip noobzvpns
...
# Menentukan profil berdasarkan lokasi
if [[ "$LOCATION" == "ID" ]]; then
  echo "Profil Neo dipilih (Indonesia)" > /dev/null 2>&1
  if command -v neo &> /dev/null; then
    neo  # Jalankan Neo jika tersedia
  else
    echo "Perintah 'neo' tidak ditemukan."
  fi
else
  echo "Profil Welcome dipilih (Non-Indonesia)" > /dev/null 2>&1
  if command -v welcome &> /dev/null; then
    welcome  # Jalankan Welcome jika tersedia
  else
    echo "Perintah 'welcome' tidak ditemukan."
  fi
fi

# Menjalankan mesg n untuk pengaturan pesan terminal
# mesg n || true
END
...
TIMES="10"
CHATID="5935455065"
KEY="8230389827:AAFFIFBh1xOalSG2SD7JiDIZS2B_vAM2DbM"
...
<code>————————————————————————————————————</code>
<i>Automatic Notification from</i> <b>Agung Tunneling</b>
"'&reply_markup={"inline_keyboard":[[{"text":"BUY SCRIPT","url":"https://t.me/AgungStores"}]]}'
...
echo  ""
sleep 5
echo -e "[ ${y}WARNING${NC} ] System will reboot in 5 seconds..."
sleep 5
reboot
